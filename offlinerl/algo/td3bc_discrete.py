...
class TD3BCD:
    ...
    def update_actor(self, obs, actions):
        ...
        alpha = alpha / q_actions.abs().mean().detach()
    ...
    def update_critic(self, obs, actions, next_obs, rewards, terminals):
        ...
        with torch.no_grad():
            ...
            next_sampled_actions = (self.actor_target(next_obs) + noise).clamp(
                0, self.action_shape - 1
            )
    ...
    def train(self, states, actions, next_states, rewards, dones, step):
        ...
        if step % self.policy_freq == 0:
            ...
            self.sync_weight(self.actor_target, self.actor, self.args.soft_target_tau)
