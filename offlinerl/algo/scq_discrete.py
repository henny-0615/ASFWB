...
from offlinerl.utils.critic import Critic_Discrete
from offlinerl.utils.tanhpolicy import PolicyNet_Discrete
...
class SCQD:
    def __init__(self, state_shape, action_shape, max_action, args):
        ...
        self.vae = VAE(state_shape, action_shape, args.vae_hidden_layer_dim, args.vae_num_hidden_layers, max_action).to(
            args.device)
        self.vae_optim = torch.optim.Adam(self.vae.parameters(), lr=args.vae_learning_rate)

        self.actor = PolicyNet_Discrete(state_shape, action_shape,
                                        args.actor_num_hidden_layers,
                                        args.hidden_layer_dim).to(args.device)
        ...
        self.critic1 = Critic_Discrete(state_shape, action_shape, args.critic_num_hidden_layers, args.hidden_layer_dim,
                                       use_ln=args.use_layernormalization).to(args.device)
        self.critic1_opt = optim.Adam(self.critic1.parameters(), lr=args.critic_learning_rate,
                                      weight_decay=args.weight_decay)
        self.critic1_target = copy.deepcopy(self.critic1)

        self.critic2 = Critic_Discrete(state_shape, action_shape, args.critic_num_hidden_layers, args.hidden_layer_dim,
                                       use_ln=args.use_layernormalization).to(args.device)
    ...
    def get_action(self, states):
        ...
        with torch.no_grad():
            a_prob = self.actor(states).cpu().data.numpy().flatten()
    ...
    def update_actor(self, obs, actions):
        ...
        sampled_actions = self.actor(obs)
        ...
        if self.args.actor_penalty_coef:
            bc_loss = (((torch.max(sampled_actions, dim=1))[0] - actions.squeeze(-1)) ** 2).sum(-1)
            policy_loss = (alpha * log_pi - q_actions + self.args.actor_penalty_coef * bc_loss).mean()
        else:
            with torch.no_grad():
                bc_loss = (((torch.max(sampled_actions, dim=1))[0] - actions) ** 2).sum(-1)
    ...
    def calculate_V(self, obs, network):
        ...
        with torch.no_grad():
            ...
            pred_Q = pred_Q.gather(1, (torch.max(behavior_actions, dim=1))[-1].to(torch.int64).unsqueeze(-1))
    ...
    def update_critic(self, obs, actions, next_obs, rewards, terminals, idd_action_threshold):
        ...
        q1_pred = q1_pred.gather(1, actions[:, 0].to(torch.int64).unsqueeze(-1))
        q2_pred = q2_pred.gather(1, actions[:, 0].to(torch.int64).unsqueeze(-1))
        ...
        with torch.no_grad():
            next_sampled_actions = self.actor(next_obs)

        # sample OOD actions
        with torch.no_grad():
            curr_sampled_actions = self.actor(obs)
        ...
        ## OOD Q1
        q1_ood_pred = self.critic1(extended_obs, extended_acs)
        q1_ood_pred = q1_ood_pred.gather(1, (torch.max(extended_acs, dim=1))[-1].to(torch.int64).unsqueeze(-1)).squeeze(
            -1)
        ...
        q2_ood_pred = q2_ood_pred.gather(1, (torch.max(extended_acs, dim=1))[-1].to(torch.int64).unsqueeze(-1)).squeeze(
            -1)
    ...
    def train(self, states, actions, next_states, rewards, dones, step):
        ...
        Step1. Update VAE
        """
        self.update_vae(states, actions.expand(-1, 5))
        with torch.no_grad():
            idd_action_threshold = self.vae.calc_dist(states, actions.expand(-1, 5)).mean()

        """
        Step2. Update critic
        """
        critic_update_data = self.update_critic(states, actions.expand(-1, 5), next_states, rewards, dones,
