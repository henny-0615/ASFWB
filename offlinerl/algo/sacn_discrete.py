...
from offlinerl.utils.critic import VectorizedCritic
...
class SACND:
    def __init__(self, state_shape, action_shape, max_action, args):
        ...
        self.critic1 = VectorizedCritic(state_shape, action_shape, args.hidden_layer_dim, num_critics=4).to(
            args.device)
        self.critic1_opt = optim.Adam(self.critic1.parameters(), lr=args.critic_learning_rate,
                                    weight_decay=args.weight_decay)
        self.critic1_target = copy.deepcopy(self.critic1)

        self.critic2 = VectorizedCritic(state_shape, action_shape, args.hidden_layer_dim, num_critics=4).to(
            args.device)
    ...
    def update_critic(self, obs, actions, next_obs, rewards, terminals):
        q1_pred = self.critic1(obs, actions.expand(-1, 5)).min(0).values
        q2_pred = self.critic2(obs, actions.expand(-1, 5)).min(0).values
